...
export function ChatPanel({ onClauseHighlight, pendingQuestion, clearPendingQuestion }: ChatPanelProps) {
  ...
  function downloadReport() {
    ...
    // Map UIMessage to the shape download helper expects
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5 line-clamp-1">{personaObj?.style}</p>
        </div>
        {messages.length > 1 && (
          <button
            onClick={downloadReport}
            className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-700 border border-slate-200 rounded-lg px-3 py-1.5 hover:border-slate-300 transition-all shrink-0"
          >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-12">
            <div className="text-4xl mb-3">{personaObj?.icon ?? '⚖️'}</div>
            <p className="text-sm font-semibold text-slate-700">{personaObj?.name}</p>
            <p className="text-xs text-slate-400 mt-1 max-w-xs leading-relaxed">{personaObj?.style}</p>
            <p className="text-xs text-slate-400 mt-4">
              Ask a question or click a clause card to start negotiating.
            </p>
          </div>
        )}
        {messages.map((m) => {
      ...
      >
        <input
          ...
          disabled={isLoading}
          className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 disabled:opacity-50"
        />
        <button
          type="submit"
